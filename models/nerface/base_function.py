...
def PE_xyz(pts, PE_fn):
    pts_PE = PE_fn(pts.reshape((-1, pts.shape[-1]))) # 2048*64x63->포지셔널 임베딩 후 각 좌표의 차원 증가3차원에서 63차원 좌표로
    return pts_PE
...
def stratified_sampling(ray_info, n_p_samples):
    ...
    #np.save('/data/private/Projects/x_save_rd',rd.cpu().numpy())
    ...
    #print(z_vals,z_vals.shape)
    # pts -> (num_rays, N_samples, #rd norm==1
    pts = co[..., None, :] + (rd[..., None, :] * z_vals[..., :, None])#2048x64x3 평면x깊이x좌표차원(3) 큐브의 좌표를 생각해보자
    #np.save('/data/private/Projects/x_sabe_rd*z',(rd[..., None, :] * z_vals[..., :, None]).cpu().numpy())
    #np.save('/data/private/Projects/x_pts_check',pts.cpu().numpy())
    #exit()

    '''
    #print(pts.shape, pts[...,-1].max(),pts[...,-1].min())
    print(rd.shape, co.shape, z_vals.shape)
    print(rd[1],co[1])
    rd_np = rd.cpu().numpy()
    co_np = co.cpu().numpy()
    pts_np = pts.cpu().numpy()
    print(pts_np.shape)
    #print([np.linalg.norm(co_np[i]) for i in range(co_np.shape[0])])
    #print(np.linalg.norm(pts_np[30000,1,:]))
    
    np.save('/data/private/Projects/x_save_pts', pts_np) 
    exit()
    #import pdb;pdb.set_trace()
    '''
    return pts, z_vals
...
def canon_ray(height: int, width: int, intrinsics , tform_cam2world: torch.Tensor, center = [0.5,0.5]):
    ...
    directions = torch.stack(#img2cam #normalized image plane으로 바꿔줌#+--/+-+/-++
        [
            ...
            torch.ones_like(ii),#DECA는 마이너스 빼래...
        ],
        dim=-1,
    ...
    #np.save('/data/private/Projects/x_save_camray',ray_directions.cpu().numpy())
    #exit()
    #여기에 cam 2 world곱해서 물체의 중심 입장에서 보는 ray direction을 알 수 있음. 실제로 모델에 넣을때는 좌표계가 바뀌면 안되기 때문에 이렇게 넣음
    #print(ray_directions)
    #print(directions[0,0,:].cpu().numpy())
    #print(ray_directions[0,0,:].cpu().numpy())
    #print(tform_cam2world[:3, -1])
    #exit()
    #print(directions[...,0].max(),ray_directions.max())
    #print(directions[...,0].min(),ray_directions.min())
    #print(directions.shape,"dir")
    #print(ray_directions.shape,"ray_dir")
    
    ray_origins = tform_cam2world[:3, -1].expand(ray_directions.shape)#translation만 알면됨.
    #print(ray_origins.shape,"rayorg")
    #print(tform_cam2world[:3, :3].shape,'tfrom')
    #print(np.linalg.norm(tform_cam2world[:3,-1].cpu().numpy()), ' :R')
    #exit()
...
def volume_rendering(radiance_field, depth_values, ray_directions, 
    ...
    return rgb_map, disp_map, acc_map, weights, depth_map, sigmas

def canonical_surface_search(radiance_field, depth_values, ray_directions, 
                                radiance_field_noise_std=0.0, background_prior = None):
    # TESTED
    #import pdb; pdb.set_trace()
    #TODO for coarse
