...
from models.nerface.base_function \
    import sample_pdf, volume_rendering, stratified_sampling, PE_and_gather, PE_xyz


def PE_forwarding(nerf, pts, rd_r, chunksize, PE4xyz, PE4d, expressions = None, latent_code = None, latent_code_t=None, wild=False, latent_trans=None, canonical = False):
...
def PE_forwarding_to_canonical(model_O2C, pts, rd, chunksize,PE4xyz,latent_code_G):

    #pts_flat = pts.reshape((-1, pts.shape[-1])) # 2048*64x3 flatening then sampling cube
    #if canonical:
        
    input_PE = PE_xyz(pts, PE4xyz)#XXX in this function points and flattened!
...
def rendering_flow(ray_info, model_coarse, model_fine, model_O2C, model_C2O, options, mode="train", encode_position_fn=None, 
                                encode_direction_fn=None, expressions = None, background_prior = None,
                                latent_code = None, latent_code_G=None):
    ...
    #XXX Observed space -> Canonical space
    canonical_points_coarse = PE_forwarding_to_canonical(
        model_O2C, 
        pts_coarse, 
        rd_reshaped, 
        params['chunksize'],
    ...
    canonical_points_coarse = \
        canonical_points_coarse.reshape(pts_coarse.shape[0],pts_coarse.shape[1], 3)
    
    # TODO torch.Size([2048, 64, 4])
    #print(output_coarse.shape,"coreara")
    # make last RGB values of each ray, the background
    assert len(pts_coarse.shape) == len(canonical_points_coarse.shape)
    ...
    #XXX positional encoding is needed?
    surface_guessing_pt = surface_intersection_search(canonical_points_coarse, weights)
