...
def PE_forwarding(nerf, pts, rd_r, chunksize, PE4xyz, PE4d, expressions = None, latent_code_E_share=None):
    ...
    #torch.Size([65536, 87]) per batch
  
    preds = [nerf(batch, expressions, latent_code_E_share) for batch in input_batches]
...
def PE_forwarding_to_canonical(model_O2C, pts, chunksize,PE4xyz):

    #pts_flat = pts.reshape((-1, pts.shape[-1])) # 2048*64x3 flatening then sampling cube
    #if canonical:
    #pts_PE = PE_fn(pts.reshape((-1, pts.shape[-1])))
    input_PE = PE4xyz(pts.reshape((-1,pts.shape[-1])))#XXX in this function points and flattened!
    
    #import pdb;pdb.set_trace()
    input_batches = [input_PE [i : i + chunksize] for i in range(0, input_PE.shape[0], chunksize)] # batch하나당 2048,87 shape chunksize인데 random_sample사이즈랑 같음
    c_pts = [model_O2C(batch) for batch in input_batches]
    c_pts = torch.stack(c_pts,axis=1)
    #TODO chunksize가 ray 갯수랑 같아서, 64개의 batch, ray가 하나의 배치라고 보는게 맞음
    #TODO it is for ablation
    return c_pts

#XXX E means environment code which shared among the same videos
#XXX G means per-frame geometry
#XXX A means per-frame appearance

def rendering_flow(ray_info, model_coarse, model_fine, model_O2C, options, mode="train", encode_position_fn=None, 
                                encode_direction_fn=None, expressions = None, background_prior = None, latent_code_E_share = None):
    ...
    canonical_points_residual = PE_forwarding_to_canonical(
        model_O2C, 
        pts_coarse, 
    ...
    #import pdb;pdb.set_trace()
    #print(canonical_points_coarse.shape,"???")
    canonical_points_coarse = canonical_points_residual + pts_coarse
    assert canonical_points_residual.shape == pts_coarse.shape 

    output_coarse = PE_forwarding(
        model_coarse, 
        canonical_points_coarse, 
        rd_reshaped, 
        params['chunksize'],
        encode_position_fn, 
        encode_direction_fn, 
    ...
    #print(output_coarse.shape,"coreara")
    ...
    (rgb_coarse, _,_, weights, _, _) = volume_rendering(
    ...
    rgb_fine, disp_fine, acc_fine, weights, depth_fine, watch_sigma = volume_rendering( # added use of weights
        output_fine,
        z_vals_full,
        rd,
        radiance_field_noise_std=params['radiance_field_noise_std'],
    ...
    print(watch_sigma.shape)
    if mode == 'val':
        pass

        #sigmas={str(i): watch_sigma[i].detach().cpu().numpy().tolist() for i in range(watch_sigma.shape[0])}
        #density_data['density(sigma)':]
        #density_data = {
        #'density(sigma)':sigmas
        #}
        #import json
        #with open('/home/nas1_userA/jaeseonglee/NerFACE_pl/outputs/density_check.json', 'w', encoding='utf-8') as file:
        #    json.dump(density_data, file, indent="\t")
...
def NerFACE(model_coarse, model_fine, model_O2C, co, rd, options, mode="train", 
                         encode_position_fn=None, encode_direction_fn=None, expressions=None, background_prior=None, latent_code_E_share=None):
    ...
    pred = [
        rendering_flow(
            ...
            latent_code_E_share = latent_code_E_share
        )
