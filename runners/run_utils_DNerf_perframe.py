...
def PE_forwarding(nerf, pts, rd_r, chunksize, PE4xyz, PE4d, expressions = None, latent_code_appr=None):
    ...
    preds = [nerf(batch, expressions, latent_code_appr) for batch in input_batches]
...
def PE_forwarding_to_canonical(model_O2C, pts, chunksize,PE4xyz,latent_code_deform):
    ...
    #import pdb;pdb.set_trace()
    input_batches = [input_PE [i : i + chunksize] for i in range(0, input_PE.shape[0], chunksize)] # batch하나당 2048,87 shape chunksize인데 random_sample사이즈랑 같음
    c_pts = [model_O2C(batch, latent_code_deform) for batch in input_batches]
...
def rendering_flow(ray_info, model_coarse, model_fine, model_O2C, options, mode="train", encode_position_fn=None, 
                                encode_direction_fn=None, expressions = None, background_prior = None, latent_code_deform = None, latent_code_appr =None):
...
def NerFACE(model_coarse, model_fine, model_O2C, co, rd, options, mode="train", 
                         encode_position_fn=None, encode_direction_fn=None, expressions=None, background_prior=None, latent_code_deform=None, latent_code_appr=None):
    ...
    pred = [
        rendering_flow(
            ...
            latent_code_deform = latent_code_deform,
            latent_code_appr = latent_code_appr
        )
