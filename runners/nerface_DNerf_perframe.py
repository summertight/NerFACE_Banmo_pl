...
from runners.run_utils_DNerf_perframe import NerFACE
...
class RunnerNerface_DNerf_perframe(pl.LightningModule):
    ...
    def __init__(self, cfg, model_pack, start_epoch):
        ...
        self.model_O2C        = model_pack['model_O2C']
        self.latent_codes_deform = model_pack['latent_codes_deform']
        self.latent_codes_appr   = model_pack['latent_codes_appr']

        # Background
        if self.cfg.dataset in ['frame', 'frame_3dmm','nerface+IM']:
            ...
            background = Image.new("RGB", (cfg.dataset['H'], cfg.dataset['W']), (0, 0, 0))
            #XXX 여기서는 black background
            background.thumbnail((cfg.dataset['H'], cfg.dataset['W']))
    ...
    def training_step(self, batch, batch_idx):
        ...
        #video_label = batch['video_label']#XXX 
        ...
        latent_code_deform = self.latent_codes_deform[data_idx].to(device)
        latent_code_appr = self.latent_codes_appr[data_idx].to(device)
        ...
        #XXX cam_o size torch.Size([2048, 3])
        ...
        rgb_coarse ,rgb_fine, _ = \
            NerFACE(self.model_coarse, self.model_fine, self.model_O2C,
                                 ...
                                 expressions = expression, background_prior=None, latent_code_deform = latent_code_deform, latent_code_appr = latent_code_appr)
        #XXX torch.Size([2048, 3]) 이런식으로 뱉음
        #XXX 지금은 None으로 bgrnd지정
        ...
        loss['latent_code_deform'] = 0.005 * torch.norm(latent_code_deform)
        loss['latent_code_appr'] = 0.005 * torch.norm(latent_code_appr)
        #loss['latent_code_E'] = 0.005 * torch.norm(latent_code_E_share)
        
        loss['total'] = loss['coarse'] + loss['fine'] + loss['latent_code_deform'] + loss['latent_code_appr']
    ...
    def validation_step(self, batch, batch_idx):
        ...
        with torch.no_grad():
            ...
            for pose in pose_list:
                ray_origins, ray_directions = canon_ray(H, W, focal, torch.from_numpy(pose.astype(np.float32)).to(device))
    ...
    def configure_optimizers(self):
        ...
        trainable_list.append(self.latent_codes_deform)
        trainable_list.append(self.latent_codes_appr)
