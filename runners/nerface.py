...
class RunnerNerface(pl.LightningModule):
    ...
    def __init__(self, cfg, model_pack, start_epoch):
        ...
        # Background
        if self.cfg.dataset['name'] in ['frame', 'frame_3dmm','nerface+IM']:
        ...
        elif self.cfg.dataset['name'] in ['frame_kakao','fc']:
            background = Image.open(os.path.join('/data/public/rw/workspace-malfo/dave','bg','00050.png'))
            background.thumbnail((cfg.dataset['H'], cfg.dataset['W']))
            self.background = torch.from_numpy(np.array(background).astype(np.float32))
    ...
    def training_step(self, batch, batch_idx):
        ...
        print(self.latent_codes.shape, "Check the number of latent codes!!")
        ...
        # Model Forward
        #if True: #XXX For voxceleb
        #    bg_color_sampled = None
