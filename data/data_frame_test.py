...
class Frame_Dataset_test(Dataset):
    def __init__(self, basedir, mode='train', img_size=[512, 512], trans_set_0=False):
        ...
        fov = 12
        aux_path = '/home/nas4_user/jaeseonglee/nerface_data_preprocess/person1'
        if mode=='train':
            mode = 'test'
            print('Swapped dataset to test')
        with open(os.path.join(basedir, f"transforms_{mode}.json"), "r") as fp0:
            self.meta_ = json.load(fp0)
        self.data_list_bbox = self.meta_['frames']
    ...
    def __getitem__(self, idx):
        ...
        #XXX R_head_vec= R_head_vec#XXX For IMAvatar Logic..?? IDK yet.6/19
        ...
        if self.mode == 'train':
            ...
            if True:
                bbox = np.array(self.data_list_bbox[idx]['bbox'])
            if True:
                pass
...
if __name__ == "__main__":
    ...
    dataset = Frame_Dataset_test(basedir=basedir, mode='val')
