...
class Frame_kakao_Dataset(Dataset):
    def __init__(self, basedir, mode='train', img_size=[512, 512], data_size=-1):
        super().__init__()
        basedir = '/data/private/Projects/dataset/dave_aux/'
        data_size=5507
        self.basedir = basedir
        self.sc_basedir = '/data/public/rw/workspace-malfo/dave/'
        self.mode = mode
        ...
        self.focal = np.array([512*8.52, 512*8.52, .5, .5])#XXX All plus is right!
    ...
    def __getitem__(self, idx):
        data_idx = idx
        idx = 'f_'+str(idx).zfill(4)
        # Load Image
        image_path = os.path.join(self.sc_basedir, self.mode ,self.data_list[idx]['file_path'] + '.png')
        ...
        pose = np.hstack((pose[:3,:3],pose[:3,-1:]/3))#scaled to 3
        exp = np.array(self.data_list[idx]['exp']).astype(np.float32)
        ...
        if self.mode == 'train':
            if 'bbox' in self.data_list[idx].keys():
                bbox = np.array(self.data_list[idx]['bbox'])/512
            else:
...
if __name__ == "__main__":
    ...
    basedir = '/data/public/rw/workspace-malfo/dave/'
    dataset = Frame_kakao_Dataset(basedir=basedir, mode='train', data_size=10)
